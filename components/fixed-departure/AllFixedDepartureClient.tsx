...
import FixedDepartureCard from '@/components/fixed-departure/FixedDepartureCard'
import type { FixedDeparturePackageCardProps } from '@/lib/fixed-departure-api'
import { DEPARTURE_STATE_VALUES, type DepartureState } from '@/lib/payload-api'
import { formatStateLabel, stateToSlug } from '@/lib/geoState'
...
// Keys are getUTCMonth() indices (0-11) as strings, not labels, so matching
// against a package's real departureDate is a direct, unambiguous lookup
// off the stored UTC date rather than the visitor's local timezone.
const DEPARTURE_MONTH_OPTIONS = [
  { key: '0',  label: 'January' },
...
// Mirrors FILTER_PER_PAGE / MAX_FILTER_FETCHES from lib/fixed-departure-api.ts's
// server-side duration loop, since departureDate's month can't be a server
// query param — this same shape of loop has to live here instead.
const MONTH_FILTER_BATCH_TARGET = 20
const MAX_MONTH_FILTER_FETCHES = 5

// Same DEPARTURE_STATE_VALUES + formatStateLabel pairing used everywhere
// else a state needs a human-readable label (detectDepartureState,
// resolveDisplayCity, the [slug] route's metadata) — sorted alphabetically
// by label since the raw enum order isn't meant for a scannable dropdown.
const STATE_OPTIONS = DEPARTURE_STATE_VALUES
  .map((state) => ({ value: state, label: formatStateLabel(state) }))
...
const SORT_OPTIONS = [
  ...
  { value: 'departure_soonest', label: 'Soonest Departure' },
...
interface Props {
  initialPackages: FixedDeparturePackageCardProps[]
...
export default function AllFixedDepartureClient({
  initialPackages,
  initialNextCursor,
  initialHasMore,
  initialTotalCount,
  ...
  const [packages, setPackages]     = useState<FixedDeparturePackageCardProps[]>(initialPackages)
  ...
  // knows how much space to reserve below it when fitting the filter lists.
  ...
  const fetchPackages = useCallback(async (targetCursor: number, mode: 'replace' | 'append') => {
    ...
    try {
      ...
      if (selectedPackageTypes.length > 0) qs.set('packageType', selectedPackageTypes.join(','))
      ...
      if (lockedState) qs.set('departureState', lockedState)
      qs.set('sort', sort)

      // departureMonth has no server-side query param (Payload only stores
      // departureDate, not a standalone month), so — same idea as the
      // duration-band loop already running server-side in
      // lib/fixed-departure-api.ts — this keeps pulling additional pages,
      // filtering each by real departureDate month client-side, until either
      // enough matches are collected, the server runs out of pages, or the
      // fetch-count safety cap is hit.
      let page = targetCursor
      let fetches = 0
      let latestCursor: number | null = null
      let latestHasMore = false
      let latestTotalCount: number | undefined
      const collected: FixedDeparturePackageCardProps[] = []

      while (true) {
        fetches++
        qs.set('cursor', String(page))
        const res = await fetch(`/api/fixed-departure?${qs.toString()}`)
        const data = await res.json()

        if (requestId !== requestIdRef.current) return

        const batch: FixedDeparturePackageCardProps[] = Array.isArray(data.packages) ? data.packages : []
        const matched = selectedDepartureMonths.length > 0
          ? batch.filter((p) => selectedDepartureMonths.includes(String(new Date(p.departureDate).getUTCMonth())))
          : batch
        collected.push(...matched)

        const batchNextCursor = typeof data.nextCursor === 'number' ? data.nextCursor : null
        const batchHasMore = Boolean(data.hasMore)
        if (latestTotalCount === undefined && typeof data.totalCount === 'number') {
          latestTotalCount = data.totalCount
        }
        latestCursor = batchNextCursor
        latestHasMore = batchHasMore

        if (collected.length >= MONTH_FILTER_BATCH_TARGET) break
        if (!batchHasMore || batchNextCursor === null) break
        if (fetches >= MAX_MONTH_FILTER_FETCHES) break
        page = batchNextCursor
      }

      setPackages((prev) => (mode === 'replace' ? collected : [...prev, ...collected]))
      setCursor(latestCursor)
      setHasMore(latestHasMore)
      if (mode === 'replace' && latestTotalCount !== undefined) {
        setTotalCount(latestTotalCount)
      }
    } finally {
      if (requestId === requestIdRef.current) setLoading(false)
    }
  }, [selectedType, selectedPackageTypes, selectedPrice, selectedDurations, selectedDepartureMonths, lockedState, sort])
  ...
  }, [selectedType, selectedPackageTypes, selectedPrice, selectedDurations, selectedDepartureMonths, sort, fetchPackages])
  ...
  function togglePackageType(key: string) {
    setSelectedPackageTypes((prev) => (prev.includes(key) ? prev.filter((k) => k !== key) : [...prev, key]))
  }
  ...
  function toggleDepartureMonth(key: string) {
    setSelectedDepartureMonths((prev) => (prev.includes(key) ? prev.filter((m) => m !== key) : [...prev, key]))
  }
  ...
  const activeFilterCount = (selectedType ? 1 : 0)
    ...
    + selectedDepartureMonths.length
  const filtersActive = activeFilterCount > 0

  const pills: { key: string; label: string; onRemove: () => void }[] = [
    ...(selectedType ? [{ key: 'type', label: selectedType, onRemove: () => setSelectedType(null) }] : []),
    ...selectedPackageTypes.map((key) => {
      const o = PACKAGE_TYPE_OPTIONS.find((opt) => opt.key === key)
      return { key: `pt-${key}`, label: o?.label ?? key, onRemove: () => togglePackageType(key) }
    }),
    ...
    ...selectedDepartureMonths.map((key) => {
      const m = DEPARTURE_MONTH_OPTIONS.find((opt) => opt.key === key)
      return { key: `month-${key}`, label: m?.label ?? key, onRemove: () => toggleDepartureMonth(key) }
    }),
  ]
        ...
        <aside className="hidden lg:block" style={{ width: '280px', flexShrink: 0 }}>
          <div
            style={{
              ...
              borderRadius: '12px',
              padding: '20px',
              maxHeight: `calc(100vh - ${HEADER_HEIGHT + FILTER_LIST_BOTTOM_BUFFER}px)`,
              overflowY: 'auto',
            }}
          >
            <FilterPanel
              ...
              selectedDepartureMonths={selectedDepartureMonths}
              onToggleDepartureMonth={toggleDepartureMonth}
...
interface FilterPanelProps {
  ...
  selectedDepartureMonths: string[]
  onToggleDepartureMonth: (key: string) => void
...
const FILTER_LIST_BOTTOM_BUFFER = 24
...
function FilterPanel({
  ...
  return (
    <div>
      ...
      {/* Departure State — a real page navigation to the chosen state's own
          route, not a filter applied in place, since each state has its own
          URL/SEO identity. Only meaningful on this state-listing view: it's
          only ever rendered here (never on the package-details view), and
          only shown at all once a lockedState is actually known. */}
      {lockedState && (
        <>
          <div style={{ marginBottom: '20px' }}>
            <p style={{ fontSize: '13px', fontWeight: 600, color: '#4A4A4A', marginBottom: '10px' }}>Departure State</p>
            <select
              value={lockedState}
              onChange={(e) => router.push(`/fixed-departure/${stateToSlug(e.target.value as DepartureState)}`)}
              style={{
                width: '100%',
                fontSize: '13px',
                fontWeight: 600,
      ...
      {/* Type */}
      <div style={{ marginBottom: '20px' }}>
        <p style={{ fontSize: '13px', fontWeight: 600, color: '#4A4A4A', marginBottom: '10px' }}>Type</p>
      ...
      {/* Package Type */}
      <div style={{ marginBottom: '20px' }}>
        <p style={{ fontSize: '13px', fontWeight: 600, color: '#4A4A4A', marginBottom: '10px' }}>Package Type</p>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
          {PACKAGE_TYPE_OPTIONS.map((o) => (
            <label key={o.key} style={{ display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer' }}>
              <input
                type="checkbox"
                checked={selectedPackageTypes.includes(o.key)}
                onChange={() => onTogglePackageType(o.key)}
                style={{ accentColor: '#1E6B2E', width: '15px', height: '15px', cursor: 'pointer' }}
              />
              <span style={{ fontSize: '13px', color: '#4A4A4A' }}>{o.label}</span>
            </label>
      ...
      {/* Departure Month */}
      <div>
        <p style={{ fontSize: '13px', fontWeight: 600, color: '#4A4A4A', marginBottom: '10px' }}>Departure Month</p>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
          {DEPARTURE_MONTH_OPTIONS.map((o) => (
            <label key={o.key} style={{ display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer' }}>
              <input
                type="checkbox"
                checked={selectedDepartureMonths.includes(o.key)}
                onChange={() => onToggleDepartureMonth(o.key)}
                style={{ accentColor: '#1E6B2E', width: '15px', height: '15px', cursor: 'pointer' }}
              />
              <span style={{ fontSize: '13px', color: '#4A4A4A' }}>{o.label}</span>
            </label>
